...
- Produces: `LoginDeps.contarFalhasRecentes(campanhaId, identificadorChave):
  Promise<number>`; `LIMITE_FALHAS`/`JANELA_MINUTOS` (constantes
  exportadas de `login.ts`). Nenhuma task futura consome isso.

- [ ] **Step 1: Escrever o teste (reescreve `login.test.ts` inteiro — preserva os 6 testes existentes, ajusta o `cpfHmac` do factory pra ser sensível ao argumento, adiciona os testes novos)**
...
describe('loginCampanha', () => {
  ...
  it('bloqueia sem chamar resolverEmailPorCpf/signIn quando falhasRecentes >= LIMITE_FALHAS, audita login.bloqueado', async () => {
    ...
    expect(d.resolverEmailPorCpf).not.toHaveBeenCalled();
    expect(d.signIn).not.toHaveBeenCalled();
    expect(d.registrarEvento).toHaveBeenCalledWith('login.bloqueado', CAMP, expect.anything());
  ...
  it('inclui identificador_chave (HMAC do CPF normalizado) no meta de uma falha existente', async () => {
    const d = deps({ signIn: vi.fn(async () => null) });
    await loginCampanha({ identificador: '529.982.247-25', senha: 'x', subdominio: 'campanha-a' }, d);
    expect(d.registrarEvento).toHaveBeenCalledWith(
      'login.falha', CAMP,
      expect.objectContaining({ motivo: 'credenciais', identificador_chave: 'hmac-52998224725' }),
    );
  });
  ...
  it('a identificador_chave é a mesma pro mesmo CPF, mesmo com motivos de falha diferentes', async () => {
...
Expected: FAIL — `contarFalhasRecentes` não existe em `LoginDeps`/não é
usado por `loginCampanha`; os testes novos falham, os 6 antigos ainda
passam (o factory `deps()` só ganhou uma propriedade extra que ainda não é
lida por ninguém).
...
export async function loginCampanha(input: LoginInput, deps: LoginDeps): Promise<{ ok: boolean }> {
  ...
  const identificadorChave = ehEmail(identificador)
    ? identificador.trim().toLowerCase()
    : deps.cpfHmac(normalizarCpf(identificador));

  const falhasRecentes = await deps.contarFalhasRecentes(campanhaId, identificadorChave);
  if (falhasRecentes >= LIMITE_FALHAS) {
    await deps.registrarEvento('login.bloqueado', campanhaId, { ip, identificador_chave: identificadorChave });
    return { ok: false };
  }

  const falha = async (motivo: string) => {
    await deps.registrarEvento('login.falha', campanhaId, { ip, motivo, identificador_chave: identificadorChave });
    return { ok: false as const };
  };
  ...
  if (ehEmail(identificador)) {
    ...
    const cpf = normalizarCpf(identificador);
    if (!cpfValido(cpf)) return falha('cpf_invalido');
    email = await deps.resolverEmailPorCpf(subdominio, identificadorChave);
    if (!email) return falha('cpf_nao_encontrado');
  }
...
Nota: no caminho CPF, `deps.resolverEmailPorCpf(subdominio, identificadorChave)`
reaproveita `identificadorChave` (já é `deps.cpfHmac(normalizarCpf(identificador))`)
em vez de chamar `deps.cpfHmac(cpf)` de novo — mesmo valor, sem
recomputar o HMAC uma segunda vez.

- [ ] **Step 4: Rodar e confirmar que passa**

Run: `cd web && npx vitest run lib/auth/login.test.ts`
Expected: PASS — 11/11 (6 preexistentes + 5 novos)
...
3 (5 falhas / 15 min, janela deslizante) → Task 1 (função) + Task 2
(constantes); decisão 4 (checagem antes de resolver CPF/chamar Auth) →
Task 2 Step 3 (`contarFalhasRecentes` é a primeira coisa depois de
resolver `campanhaId`); decisão 5 (mensagem genérica, sem revelar
bloqueio) → Task 2 (rota HTTP não muda, `POST /api/auth/login` já
retorna sempre a mesma mensagem pra `{ok:false}`); decisão 6
(`'login.bloqueado'` não conta) → Task 1 Step 4-5 (fixture prova isso
explicitamente) + Task 2 (`registrarEvento('login.bloqueado', ...)`);
...
(índice na mesma migration) → Task 1 Step 1+6. Os 13 itens de teste do
spec → cobertos: 1-4 (Task 2), 5-11 (Task 1), 12-13 (Task 3).
...
string) => Promise<number>`. O nome do parâmetro RPC
